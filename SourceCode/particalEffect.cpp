...
HRESULT InitParticalEffect(void)
{
	...
	//頂点バッファの生成
	if (FAILED(pD3DDevice->CreateVertexBuffer(sizeof(VERTEX_3D)*NUM_PARTICALEFFECTVERTEX*MAX_PARTICALEFFECT, D3DUSAGE_WRITEONLY, FVF_VERTEX2D, D3DPOOL_MANAGED, &g_pVtxBufferParticalEffectPolygon, NULL)))
	{
		return E_FAIL;
	}
...
void DrawParticalEffect(void)
{
	...
	//エフェクト：頂点フォーマット設定
	pD3DDevice->SetStreamSource(0, g_pVtxBufferParticalEffectPolygon, 0, sizeof(VERTEX_3D));              //VRAMからGPUにデータをストリーム
	...
	for (int nCnt = 0; nCnt < MAX_PARTICALEFFECT; nCnt++)
	{
		...
		if (g_ParticalEffect[nCnt].bUse == true)
		{
			pD3DDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, nCnt*4, 2);                        //テクスチャメモリの確保
		}
	}
...
void MakeVertexPolygonParticalEffect(LPDIRECT3DDEVICE9 pDevice, VERTEX_3D *pVtx)
{
